...
This step implements real user registration for Spendly. The `/register` page and form already exist (`templates/register.html` posts `name`, `email`, `password` to `POST /register`), but `app.py` only has a `GET /register` route that renders the template — there is no handler for the form submission. This step adds the `POST /register` route: validate input, hash the password, insert a new row into the `users` table, and start a logged-in session for the new user. It builds directly on the database layer from Step 1 and is the foundation every later auth-gated feature (profile, expenses) depends on.
...
- `POST /register` — validate submitted fields, create the user, log them in, redirect to profile — public
...
- **Create:** none
- **Modify:** `templates/register.html` — no structural changes required; it already posts to `/register` and renders `{{ error }}`. Only re-render with `error` and previously entered `name`/`email` values on validation failure (e.g. `value="{{ name or '' }}"` on the inputs) so the user doesn't retype everything.
...
- `templates/register.html` — repopulate `name`/`email` values on re-render after a validation error.
...
- [ ] Submitting with a missing field or password under 8 characters re-renders the form with an error instead of crashing
